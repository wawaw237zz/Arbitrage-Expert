...
#property description "billyabcede@gmail.com"
#property strict
...
#define EA_NAME "Locking System Expert"
...
string gTradingPanelObjects[150];
...
void TakeTrade(int direction)
{
   ...
   if(direction == 1)
   {
       ...
       gBuyTicket = OpenMarketOrder(Symbol(), OP_BUY, gLotSize, "Long Order", clrGreen);
   ...
   {
       gSellTicket = OpenMarketOrder(Symbol(), OP_SELL, gLotSize, "Short Order", clrRed);
       if(OrderSelect(gSellTicket, SELECT_BY_TICKET))
       {
          ...
          //open a recovery stop order in the opposite direction
          gLotSize = GetTradeVolume(2);
          gBuyStopTicket = OpenPendingOrder(Symbol(), OP_BUYSTOP, gLotSize, gBuyOpenPrice, 0, 0, "Recovery Buy Stop)", 0, clrTurquoise);
          gCurrentDirection = direction;
...
void CreateTradingPanel()
{
   ...
   ObjectSetString(0, sellButtonName, OBJPROP_TEXT, "SHORT");
   ...
   ObjectSetString(0, buyButtonName, OBJPROP_TEXT, "LONG");
   ...
   ObjectSetString(0, CLOSE_ALL_BUTTON, OBJPROP_TEXT, "Close All Trades");
